            ...
            if licFile.is_file():
                with open(licFile.path, 'r', errors='ignore') as f:
                    ...
                    if commonLicenseName:
                        licenseText += f'License-Text: <a href="#{commonLicenseName}">{commonLicenseName}</a>\n'
                    else:
    ...
    with open(args.output, 'w') as f:
        ...
        if args.jar_license_info:
            for jarLicInfoFile in args.jar_license_info:
                ...
                if os.path.exists(jarLicInfoFile):
                    with open(jarLicInfoFile, 'r', errors='ignore') as fJarLic:
                        jarTitle = os.path.basename(jarLicInfoFile).replace('-JARLICENSEINFO.txt', '')
                        content = fJarLic.read()
                        f.write(f'<h2>License Information from {jarTitle}</h2>\n')
                        f.write('<pre>' + content + '</pre>\n')       

        log.info('Writing common licenses...')
        f.write('<h2>Licenses</h2>\n')
        for licName, licHash in commonLicensesDict.items():
            f.write(f'<h3 id="{licName}">{licName}</h3>\n')
            with open(os.path.join(os.path.dirname(__file__), 'common-licenses', licName), 'r', errors='ignore') as fLic:
                content = fLic.read()
                f.write('<pre>' + content + '</pre>\n')
        f.write('</body></html>\n')
