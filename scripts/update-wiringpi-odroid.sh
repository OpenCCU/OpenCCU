...
ID=${1:-$(resolve_latest_github_head_commit "hardkernel" "wiringPi")}
PACKAGE_NAME="wiringpi-odroid"
PROJECT_URL="https://github.com/hardkernel/wiringPi"
ARCHIVE_URL="${PROJECT_URL}/archive/${ID}/${PACKAGE_NAME}-${ID}.tar.gz"
CURRENT_ID=$(sed -nE 's/^WIRINGPI_ODROID_VERSION = (.*)$/\1/p' "buildroot-external/package/${PACKAGE_NAME}/${PACKAGE_NAME}.mk" | head -n1)
