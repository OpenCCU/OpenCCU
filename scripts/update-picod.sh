...
ID=${1:-$(resolve_latest_github_head_commit "ef-gy" "rpi-ups-pico")}
PACKAGE_NAME="picod"
PROJECT_URL="https://github.com/ef-gy/rpi-ups-pico"
ARCHIVE_URL="${PROJECT_URL}/archive/${ID}/${PACKAGE_NAME}-${ID}.tar.gz"
CURRENT_ID=$(sed -nE 's/^PICOD_VERSION = (.*)$/\1/p' "buildroot-external/package/${PACKAGE_NAME}/${PACKAGE_NAME}.mk" | head -n1)
...
if [[ -n "${ARCHIVE_HASH}" ]]; then
  # update package info
  sed -i "s/${PACKAGE_NAME^^}_VERSION = .*/${PACKAGE_NAME^^}_VERSION = ${ID}/g" "buildroot-external/package/${PACKAGE_NAME}/${PACKAGE_NAME}.mk"
  # update package hash
